...
func TestReadGroupBTreeEntries_NonLeafNode(t *testing.T) {
	...
	// Create root node (level 1, non-leaf)
	copy(buf[0:4], "TREE")
	buf[4] = 0
	buf[5] = 1 // Level 1 (non-leaf)
	binary.LittleEndian.PutUint16(buf[6:8], 1)

	// Left/right siblings (8 bytes each, offsetSize=8)
	binary.LittleEndian.PutUint64(buf[8:16], 0xFFFFFFFFFFFFFFFF)
	binary.LittleEndian.PutUint64(buf[16:24], 0xFFFFFFFFFFFFFFFF)

	// Key[0] = 0, Child[0] = 100 (points to child B-tree node)
	binary.LittleEndian.PutUint64(buf[24:32], 0)
	binary.LittleEndian.PutUint64(buf[32:40], 100)

	// Create child B-tree node at offset 100 (level 0, leaf)
	childOffset := 100
	copy(buf[childOffset:childOffset+4], "TREE")
	buf[childOffset+4] = 0
	buf[childOffset+5] = 0 // Level 0 (leaf)
	binary.LittleEndian.PutUint16(buf[childOffset+6:childOffset+8], 1)

	// Left/right siblings
	binary.LittleEndian.PutUint64(buf[childOffset+8:childOffset+16], 0xFFFFFFFFFFFFFFFF)
	binary.LittleEndian.PutUint64(buf[childOffset+16:childOffset+24], 0xFFFFFFFFFFFFFFFF)

	// Key[0] = 0, Child[0] = 200 (points to SNOD)
	binary.LittleEndian.PutUint64(buf[childOffset+24:childOffset+32], 0)
	binary.LittleEndian.PutUint64(buf[childOffset+32:childOffset+40], 200)

	// Create SNOD at offset 200
	snodOffset := 200
	copy(buf[snodOffset:snodOffset+4], "SNOD")
	buf[snodOffset+4] = 1
	buf[snodOffset+5] = 0
	binary.LittleEndian.PutUint16(buf[snodOffset+6:snodOffset+8], 1)

	// Entry: link offset=0, object address=300, cache type=0
	binary.LittleEndian.PutUint64(buf[snodOffset+8:snodOffset+16], 0)
	binary.LittleEndian.PutUint64(buf[snodOffset+16:snodOffset+24], 300)
	binary.LittleEndian.PutUint32(buf[snodOffset+24:snodOffset+28], 0)
	binary.LittleEndian.PutUint32(buf[snodOffset+28:snodOffset+32], 0)
